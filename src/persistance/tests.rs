mod answers_test {
    ...
    use crate:: {
        ...
        persistance::{
            answers_doa::{AnswersDao, AnswersDaoImpl},
            questions_doa::{QuestionsDao, QuestionsDaoImpl},
        }
    };
    ...
    async fn create_answer_should_fail_with_malformed_uuid(pool: PgPool) -> Result<(), String> {
        ...
        if result.is_ok() {
            return Err(format!("Expected an error but got the following result: {:?}", result.unwrap();))
        } 
        ...
        if let Err(DBError::InvalidUUID(_)) = result {
            ...
            Err(format!("Expected an invalid UUID error but got the following error: {:?}", result.err()
            ))
        }
    }    
    ...
    #[sqlx::test]
    async fn create_answer_should_fail_if_database_error_occurs(pool: PgPool) -> Result<(), String> {
        ...
        if let Err(DBError::Other(_)) = result {
            ...
            Err(format!("Expected an Other error but got the following error: {:?}", result.err()
            ))
        }
    }
    ...
    async fn create_answer_should_succeed(pool: PgPool) -> Result<(), String> {
        ...
        let answer_doa = Answers_doaImpl::new(pool);
    ...
    async fn delete_answer_should_fail_with_malformed_uuid(pool: PgPool) -> Result<(), String> {
        ...
        if let Err(DBError::InvalidUUID(_)) = result {
            ...
            Err(format!("Expected an invalid UUID but got the following error: {:?}", result.err()
            ))
        }
    }

    #[sqlx::test]
    async fn delete_answer_should_fail_if_database_error_occurs(pool: PgPool) -> Result<(), String> {
        let answer_doa = AnswerasDaoImpl::new(pool.clone());
        pool.close().await;
        let result = answer_doa
    ...
    async fn get_answers_should_fail_if_database_error_occurs(pool: PgPool) -> Result<(), String> {
        ...
        let result = answer_doa
            ...
            .await;
        if result.is_ok() {
            Err(format!("Expected an error but got the following result: {:?}", result.unwrap()
            ))
        } else {
            Err(format!("Expected an Other Error but got the following error: {:?}", result.err()
            ))
    ...
    #[sqlx::test]
    async fn get_answers_should_succedd(pool: PgPool) -> Result<(), String> {
        let question_doa = QuestionsDaoImpl::new(pool.clone());
        let answer_doa = AnswersDaoImpl::new(pool);
        let question = question_doa
            .create_question( Question {
        ...
        let results = answers_doa
            .get_answers(question.question_uuid.clone())
            .await
            .map_err(|e| format!("{:?}", e))?;

        if results.len() != 1 {
            return Err("Incorrect number of results returns".to_owned());
        }
...
mod questions_tests {
    ...
    use crate:: {
        ...
        persistance::questions_doa::{QuestionsDao, QuestionsDaoImpl}
    };

    #[sqlx::test]
    async fn create_question_should_fail_if_database_error_occurs(pool: PgPool) -> Result<(), String> {
        ...
        if result.is_ok() {
            return Err(format!("Expected an error but got the followoing result: {:?}", result.unwrap()
            ))
        }
        if let Err(DBError::Other(_)) = result {
            Ok(())
   ...
   async fn delete_question_should_fail_with_malformed_uuid(pool: PgPool) -> Result<(), String> {
    ...
    if result.is_ok() { return Err(format!(
   ...
   #[sqlx::test]
   async fn delete_question_should_fail_if_database_error_occurs(pool: PgPool) -> Result<(), String> {
    let doa = QuestionsDaoImpl::new(pool.clone());
    pool.close().await;
    let result = doa
   ...
   async fn delete_question_should_succeed(pool: PgPool) -> Result<(), String> {
    let doa = QuestionsDaoImpl::new(pool);
    let result = doa.create_question(Question {
        title: "test title".to_owned(),
        description: "test description".to_owned(),
    })
    .await
    .map_err(|e| format!("{:?}, e"))?;
    let results = doa.get_answers().await.map_err(|e| format!("{:?}", e))?;
    if results.len() != 0 {
   ...
   #[sqlx::test]
   async fn get_questions_should_fail_if_database_error_occurs(pool: PgPool) -> Result<(), String> {
    let doa = QuestionsDaoImpl::new(pool);
    pool.close().await;
   ...
   async fn get_questions_should_succeed(pool: PgPool) -> Result<(), String> {
    let doa = QuestionsDaoImpl::new(pool);
    let result = doa.create_questions(Questions {
        title: "test title".to_owned(),
        description: "test description".to_owned(),
        })
        .await
    ...
    if results.get(0).unwrap().questions_uuid != result.question_uuid {
